...
try:
    ...
    print('\nСистеме не удалось прочитать файл базы данных. Данные повреждены ввиду ручного вмешательства '
          'во внутренние файлы системы или следствием повреждения данных.\n')
    database_dict = {}
        ...
        def list_best_students(database_local, group_number):
            ...
            for students in database_local[group_number]:
                ...
                if average_score > 5:
                    ...
                    print('\nВ группе ещё нет студентов со средним балом выше 6\n')
...
def editing_space(database_main):
    ...
    while True:
        ...
        def checking_user_action(group_number):
            while True:
                print("Введите \"1\" для редактирования группы или \"2\" ДЛЯ редактирования студентов группы", group_number)
                try:
                    ...
                    def renaming_group_number():
                        ...
                        while True:
                            ...
                            if enter_new_group_number in database_local:
                                print("\nИзменить группу невозможно т.к. она уже существует\n")
                                continue
                    ...
                    def deleting_marks(status):
                        while status == "R":
                            ...
                            try:
                                enter_delete_mark = int(input("Введите оценку которую хотите удалить: "))
                                if len(database_local[group_number][student_id_local]) == 0:
                                    print("\nУ студента нету ни одной отметки. Удаление невозможно. "
                                          "Для начала добавьте студенту отметки.\n")
                                    print(database_dict)
                                    continue
